...
class Nodes:
    ...
    def categorize_email(self, state: GraphState) -> GraphState:
        ...
        # Print email subject for debugging
        print(Fore.CYAN + "📩 Processing Email:" + Style.RESET_ALL)
        print("   Subject:", current_email.subject)
        print("   From:", getattr(current_email, "sender", "N/A"))
        print()

        # Categorize
        result = self.agents.categorize_email.invoke({"email": current_email.body})
        print(Fore.MAGENTA + f"Email category: {result.category.value}\n" + Style.RESET_ALL)
    ...
    def skip_unrelated_email(self, state):
        ...
        print("❌ Skipped Subject:", email.subject, "\n")
        state["emails"].pop()
        return state
